...
export function formatPageInfo(data) {
    if (!data)
        return "Keine Seiteninformationen.";
    return [
        `Seite: ${data.name || "–"}`,
        `ID: ${data.id || "–"}`,
...
export function formatCustomFields(fields) {
    if (!fields?.length)
        return "Keine Custom Fields vorhanden.";
    return `${fields.length} Custom Fields:\n` + fields.map((f) => `  • ${f.name} [${f.type}]${f.description ? ` — ${f.description}` : ""} (ID: ${f.id})`).join("\n");
}
export function formatBotFields(fields) {
    if (!fields?.length)
        return "Keine Bot Fields vorhanden.";
    return `${fields.length} Bot Fields:\n` + fields.map((f) => `  • ${f.name} [${f.type}] = ${f.value ?? "–"}${f.description ? ` — ${f.description}` : ""} (ID: ${f.id})`).join("\n");
}
export function formatFlows(data) {
    const flows = data?.flows;
    const folders = data?.folders;
    if (!flows?.length)
        return "Keine Flows vorhanden.";
...
export function formatGrowthTools(tools) {
    if (!tools?.length)
...
export function formatOtnTopics(topics) {
    ...
    return `${topics.length} OTN Topics:\n` + topics.map((t) => `  • ${t.name}${t.description ? ` — ${t.description}` : ""} (ID: ${t.id})`).join("\n");
}
export function formatSubscriber(data) {
    if (!data)
        return "Kein Subscriber gefunden.";
    const lines = [
    ...
    if (data.tags?.length) {
        lines.push(`Tags: ${data.tags.map((t) => t.name).join(", ")}`);
    }
    if (data.custom_fields?.length) {
        const filled = data.custom_fields.filter((f) => f.value !== null && f.value !== undefined && f.value !== "");
        if (filled.length) {
...
export function formatSubscribers(subscribers) {
    if (!subscribers?.length)
        return "Keine Subscriber gefunden.";
    return `${subscribers.length} Subscriber:\n` + subscribers.map((s) => {
        ...
        if (s.phone)
            parts.push(s.phone);
        if (s.tags?.length)
            parts.push(`Tags: ${s.tags.map((t) => t.name).join(", ")}`);
        return `  • ${parts.join(" | ")}`;
    }).join("\n");
}
