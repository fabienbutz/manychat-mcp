...
async function fetchList(endpoint: string): Promise<any[]> {
  ...
  const list = (result.data as unknown as any[]) ?? [];
  cache.set(endpoint, { data: list, fetchedAt: Date.now() });
  return list;
}

export function invalidateResolverCache(endpoint?: string): void {
...
function caseInsensitiveMatch(
  ...
  if (hits.length > 1) {
    ...
    throw new Error(
      `Mehrdeutig: ${hits.length} ${kind}s matchen case-insensitiv auf "${query}": ${names}. Bitte mit exakter Schreibweise oder ID aufrufen.`,
    );
  }
  const available = list.map((x) => `"${x[nameKey]}"`).join(", ") || "(keine vorhanden)";
  throw new Error(
    `${kind} "${query}" nicht gefunden. Verfügbare ${kind}s: ${available}. Erstelle ihn ggf. zuerst oder prüfe die Schreibweise.`,
  );
}

export async function resolveTagId(name: string): Promise<Resolution> {
  const list = await fetchList("/fb/page/getTags");
  return caseInsensitiveMatch(list, "name", "id", name, "Tag");
}

export async function resolveCustomFieldId(name: string): Promise<Resolution> {
  const list = await fetchList("/fb/page/getCustomFields");
  return caseInsensitiveMatch(list, "name", "id", name, "Custom Field");
}

export async function resolveBotFieldId(name: string): Promise<Resolution> {
