...
export function registerSendingTools(server: McpServer) {
  server.tool(
    "send_content",
    "Send dynamic content (text, images, videos, cards, buttons) to a ManyChat subscriber. Supports up to 10 messages, 5 actions, and 11 quick replies per request.",
    {
  ...
  server.tool(
    "send_text",
    "Send a simple text message to a ManyChat subscriber. Shortcut for send_content with a single text message.",
    {
      subscriber_id: z.string().describe("Subscriber ID (required)"),
